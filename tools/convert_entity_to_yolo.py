"""
将 Live Box Annotator 标注的实体 JSON 转换为 YOLO 数据集
类别:
  0: Player
  1: Monster
  2: HP
...
OUT_DIR = Path("data/yolo_entity_dataset")

# Classes mapped to IDs
CLASS_MAP = {
    "Player": 0,
    "Monster": 1,
    "HP": 2,
...
groups = []
# The live_box_annotator saved json files with simply .json instead of _labels.json 
# because prefix was formatted differently, let's look at the filenames.
for f in sorted(SNAP_DIR.glob("*.json")):
    prefix = f.stem
    raw_img = SNAP_DIR / f"{prefix.replace('_labels', '')}_raw.png"
    if not raw_img.exists():
        # Let's try matching exactly by name since the code was: prefix = ts_idx
...
print(f"[Data] Found {len(groups)} labeled images")
...
def augment_image(img, labels_json, img_w, img_h, aug_type):
    ...
    if aug_type == "brightness":
        ...
        # Cannot flip easily for HP/MP or asymmetrical UI, just shift
...
for raw_path, labels in groups:
    ...
    has_hp_mp = any(b["cls"] in ["HP", "MP"] for b in labels.get("boxes", []))
    n_augs = 120 if has_hp_mp else 10
...
print(f"Train/Val: {split_idx} / {len(all_samples)-split_idx}")
